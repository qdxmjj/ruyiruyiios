...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [self.dcycleView addSubview:self.deleteBtn];
    }
    return self;
}
...
-(void)layoutSubviews{
    ...
    [self.dcycleView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo((self.frame.size.width-32)*1.1);
    }];
    
    [self.deleteBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.bottom.mas_equalTo(self.dcycleView.mas_bottom);
...
-(void)cycleScrollView:(SDCycleScrollView *)cycleScrollView didSelectItemAtIndex:(NSInteger)index{
    ...
    NSString *webUrl = [NSString stringWithFormat:@"%@",[self.activityArr[index] objectForKey:@"webUrl"]];
    ...
    if ([clickStatus isEqualToString:@"1"]) {
        ...
        if ([shareStatus isEqualToString:@"1"]) {
            ...
            if ([self.delegate respondsToSelector:@selector(adview:didSelectItemAtShareType:shareText:shareURL:)]) {
                
                [self.delegate adview:self didSelectItemAtShareType:shareStatusAble shareText:text shareURL:webUrl];
            }
        }else{
            
            if ([self.delegate respondsToSelector:@selector(adview:didSelectItemAtShareType:shareText:shareURL:)]) {
                
                [self.delegate adview:self didSelectItemAtShareType:shareStatusNotAble shareText:@"" shareURL:webUrl];
            }
        }
    }else{
        NSLog(@"不可以点击！");
...
-(SDCycleScrollView *)dcycleView{
    ...
    if (!_dcycleView) {
        ...
        _dcycleView.backgroundColor = [UIColor whiteColor];
