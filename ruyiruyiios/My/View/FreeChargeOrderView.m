...
@implementation FreeChargeOrderView
...
- (UILabel *)serProcessLabel{
    
    if (_serProcessLabel == nil) {
        
        _serProcessLabel = [[UILabel alloc] init];
        _serProcessLabel.font = [UIFont fontWithName:TEXTFONT size:16.0];
        _serProcessLabel.textColor = [UIColor lightGrayColor];
        _serProcessLabel.textAlignment = NSTextAlignmentRight;
    }
    return _serProcessLabel;
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        NSArray *nameArray = @[@"联系人", @"联系电话", @"车牌号", @"服务项目", @"服务进程"];
...
- (void)addChangeViews{
    ...
    [self addSubview:self.serProcessLabel];
...
- (void)layoutSubviews{
    ...
    self.serProcessLabel.frame = CGRectMake(MAINSCREEN.width/2, 155, MAINSCREEN.width/2 - 20, 20);
    self.typeLabel.frame = CGRectMake(20, 190, MAINSCREEN.width/2 - 20, 20);
    self.storeNameBtn.frame = CGRectMake(MAINSCREEN.width/2, 190, MAINSCREEN.width/2 - 20, 20);
    self.underView.frame = CGRectMake(0, 223, MAINSCREEN.width, 1);
}
